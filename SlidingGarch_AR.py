...
class SlidingGARCH:
    ...
    def FHS(self, ticker):
      ...
      for t in range(window, len(returns)):
          ...
          # 用 sigma_t 的索引对齐 returns
          returns_aligned = window_returns.loc[sigma_t.index]

          # 然后做标准化和回调
          standardized_returns = returns_aligned / sigma_t
    ...
    def run_sliding_window(self, ticker):
        ...
        for i in range(window, n):
            ...
            cv    = res.conditional_volatility      # 一个 pd.Series
            resid = res.resid                       # 一个 pd.Series
    ...
    def plot_last_qq(self, ticker):
        ...
        plt.legend()
        plt.savefig(f"QQ-{ticker}-{sliding_model.vol}-{sliding_model.mean}-{sliding_model.o}.png")
        #plt.show()
